...
form.querySelector(`input[name="serverMode"][value="${stored.serverMode || DEFAULT_OPTIONS.serverMode}"]`).checked = true;
for (const input of form.querySelectorAll('input[name="formats"]')) {
  input.checked = (stored.formats || DEFAULT_OPTIONS.formats).includes(input.value);
}
...
form.addEventListener('submit', async (event) => {
  ...
  const formats = [...form.querySelectorAll('input[name="formats"]:checked')].map((input) => input.value);
  if (!formats.length) {
...
testServer.addEventListener('click', async () => {
  try {
    const response = await chrome.runtime.sendMessage({ type: 'server:status' });
    if (!response?.ok) {
    ...
    const idle = response.server?.idleMs ? ` idle=${Math.round(response.server.idleMs / 1000)}s` : '';
