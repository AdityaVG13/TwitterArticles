...
import { renderMarkdown, saveArticleFiles, zipFiles } from '../src/exporters.mjs';
...
test('saveArticleFiles can write pdf when enabled', { skip: process.env.XAD_TEST_PDF !== '1' }, async () => {
  const dir = await mkdtemp(path.join(os.tmpdir(), 'xad-'));
  try {
