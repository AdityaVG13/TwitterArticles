...
import { fileStemForArticle, normalizeSourceUrl, parseFormats, parseUrlInput } from '../src/url.mjs';

test('parseUrlInput accepts pasted batches', () => {
  assert.deepEqual(parseUrlInput('https://x.com/a/status/1,\n<https://twitter.com/b/status/2>'), [
...
test('normalizeSourceUrl canonicalizes twitter.com to x.com', () => {
  assert.equal(normalizeSourceUrl('https://twitter.com/a/status/1#ignored'), 'https://x.com/a/status/1');
});

test('normalizeSourceUrl rejects non-X hosts by default', () => {
  assert.throws(() => normalizeSourceUrl('https://example.com/article'), /Only x.com/);
});
...
test('fileStemForArticle is stable and filesystem safe', () => {
  const stem = fileStemForArticle({ title: 'A/B: Useful Article?', sourceUrl: 'https://x.com/a/status/1' });
